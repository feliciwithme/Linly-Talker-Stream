...
_musetalk_dir = os.path.dirname(_utils_dir)
_avatars_dir = os.path.dirname(_musetalk_dir)
_src_dir = os.path.dirname(_avatars_dir)
...
def load_all_model(
    ...
    if unet_model_path is None:
        unet_model_path = os.path.join(PROJECT_ROOT, "models", "musetalk", "musetalkV15", "unet.pth")
    elif not os.path.isabs(unet_model_path):
        unet_model_path = os.path.join(PROJECT_ROOT, unet_model_path)
    
    if unet_config is None:
        unet_config = os.path.join(PROJECT_ROOT, "models", "musetalk", "musetalkV15", "musetalk.json")
    elif not os.path.isabs(unet_config):
        unet_config = os.path.join(PROJECT_ROOT, unet_config)
    
    # VAE model path - use absolute path
