...
_face_parsing_dir = os.path.dirname(_current_file)
_utils_dir = os.path.dirname(_face_parsing_dir)
...
class FaceParsing():
    ...
    def model_init(self, 
        ...
        elif not os.path.isabs(resnet_path):
            resnet_path = os.path.join(PROJECT_ROOT, resnet_path)
            
        if model_pth is None:
            model_pth = os.path.join(PROJECT_ROOT, 'models/face-parse-bisent/79999_iter.pth')
        elif not os.path.isabs(model_pth):
            model_pth = os.path.join(PROJECT_ROOT, model_pth)
